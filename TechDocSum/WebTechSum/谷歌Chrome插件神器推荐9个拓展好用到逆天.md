[toc]
这篇文章纯粹为视频贴提纲链接而已，如果想看用法，推荐看视频。

[谷歌 Chrome 拓展推荐（好用到爆炸），每一个都无法被取代，浏览器必备神器](https://youtu.be/di1J5GAbSm4)

前面几期视频中我分享了怎样安装Chrome浏览器，和怎么下载插件，其实这些都是为了这期视频做铺垫，也就是分享一些 Chrome 浏览器的必备插件神器。

这里我刻意强调必备原因是，好用的插件实在太多了，如果我这期视频分享几十个或者上百个，那也没意义。毕竟每个人情况不同，很多插件的作用方向太垂直，只针对某一个小众的群体。

大部分人并不适用。所以我只推荐 必备 ，**也就是不管是你哪个行业的，做什么工作**。只要使用 Chrome 浏览器，装上这些插件，绝对让你的Chrome浏览器要用到爆炸。

### **1、谷歌访问助手**

无需搭建 VPN，只要安装此插件，就能使用 Google 搜索、Gmail、Chrome网上应用商店等。该插件支持免费永久使用。

[https://github.com/haotian-wang/google-access-helper](https://github.com/haotian-wang/google-access-helper)

### **2、IE Tab**

在 Chrome 浏览器中使用 IE 内核

[https://chrome.google.com/webstore/detail/ie-tab/hehijbfgiekmjfkfjpbkbammjbdenadd](https://chrome.google.com/webstore/detail/ie-tab/hehijbfgiekmjfkfjpbkbammjbdenadd)

### **3、Adblock Plus**

广告拦截，支持自定义规则，视频中，我添加了自定义规则，只过滤拦截百度广告，放开谷歌广告。

[https://chrome.google.com/webstore/detail/adblock-plus-free-ad-bloc/cfhdojbkjhnklbpkdaibdccddilifddb](https://chrome.google.com/webstore/detail/adblock-plus-free-ad-bloc/cfhdojbkjhnklbpkdaibdccddilifddb)

只拦截百度广告，不拦截谷歌广告，自定义规则。

[如何只屏蔽百度广告 放开谷歌广告，添加自定义拦截规则](https://guozh.net/only-block-baidu-ad/)

### **4、crxMouse Chrome-鼠标手势**

[https://chrome.google.com/webstore/detail/crxmouse-chrome-gestures/jlgkpaicikihijadgifklkbpdajbkhjo](https://chrome.google.com/webstore/detail/crxmouse-chrome-gestures/jlgkpaicikihijadgifklkbpdajbkhjo)

### **5、Chrome Better History – 查看浏览器历史记录，支持跳转时间日期**

[https://chrome.google.com/webstore/detail/chrome-better-history/aadbaagbanfijdnflkhepgjmhlpppbad](https://chrome.google.com/webstore/detail/chrome-better-history/aadbaagbanfijdnflkhepgjmhlpppbad)

### **6、Toby for Chrome – 效率提升工具，管理标签tab**

[https://chrome.google.com/webstore/detail/toby-for-chrome/hddnkoipeenegfoeaoibdmnaalmgkpip](https://chrome.google.com/webstore/detail/toby-for-chrome/hddnkoipeenegfoeaoibdmnaalmgkpip) [https://chrome.google.com/webstore/detail/toby-mini/gfdcgfhkelkdmglklfbndgopaihmoeci](https://chrome.google.com/webstore/detail/toby-mini/gfdcgfhkelkdmglklfbndgopaihmoeci)

### **7、SimpleExtManager – 管理拓展插件，支持开启、关闭、卸载**

[https://chrome.google.com/webstore/detail/simpleextmanager/kniehgiejgnnpgojkdhhjbgbllnfkfdk](https://chrome.google.com/webstore/detail/simpleextmanager/kniehgiejgnnpgojkdhhjbgbllnfkfdk)

### **8、图片助手(ImageAssistant) – 批量图片下载**

[https://chrome.google.com/webstore/detail/imageassistant-batch-imag/dbjbempljhcmhlfpfacalomonjpalpko](https://chrome.google.com/webstore/detail/imageassistant-batch-imag/dbjbempljhcmhlfpfacalomonjpalpko)

好像应用商店已经下架这插件，我将自己的打包到百度网盘，有需要的搜索微信公众号「老郭种树」，然后后台输入「图片」，就能获取下载地址。

### **9、达达划词翻译**

[https://chrome.google.com/webstore/detail/cajhcjfcodjoalmhjekljnfkgjlkeajl](https://chrome.google.com/webstore/detail/cajhcjfcodjoalmhjekljnfkgjlkeajl)

### 整个Chrome插件系列其他视频：

#### **1)、谷歌 Chrome 插件推荐，针对 IT 程序员的拓展程序（珍藏已久）**

[https://www.youtube.com/watch?v=t-7dc85evAo](https://www.youtube.com/watch?v=t-7dc85evAo)

#### **2)、Chrome 插件分享，自媒体必备“神器”，拒绝踩坑**

[https://youtu.be/6JI_ZdO3-Ho](https://youtu.be/6JI_ZdO3-Ho)

[https://youtu.be/K1CO9kPvAUA](https://youtu.be/K1CO9kPvAUA)

**怎么说呢？不管有用没，还是想加上这句**  
**[老郭种树](https://guozh.net/)原创，转载请加上[谷歌 Chrome 插件神器推荐，这 9 个拓展好用到逆天](https://guozh.net/google-chrome-extensions-1/)**

<img width="700" height="386" src="../_resources/d5CP2qs7EcFTkXh_8e1a469b60f44ed28249c5eebea01372.png"/>

  

### 向你推荐:

- [分享几款常用的 Chrome 插件 YouTube 、哔哩哔哩](https://guozh.net/chrome-extensions-for-youtube/ "分享几款常用的 Chrome 插件 YouTube 、哔哩哔哩")
- [Chrome 插件推荐之程序员](https://guozh.net/chrome-extensions-for-coder/ "Chrome 插件推荐之程序员")
- [自媒体人用的 Chrome 插件神器](https://guozh.net/chrome-extension-for-wemedia/ "自媒体人用的 Chrome 插件神器")
- [Chrome 插件卸载后，还在生效，删除干净的两个步骤](https://guozh.net/chrome-delete-uninstall-complete/ "Chrome 插件卸载后，还在生效，删除干净的两个步骤")