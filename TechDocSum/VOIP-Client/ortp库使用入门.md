...
（4）基于GPL版权声明。
ORTP可以在其[官方网站](http://www.linphone.org/index.php/eng/code_review/ortp)上下载，下载解压后得到ORTP的源码包和示例程序（tests）。其帮助文档在docs目录下，也可以在[DOCS](http://download.savannah.gnu.org/releases/linphone/ortp/docs/)在线查看。
...
关于rtp session的管理和调度，由全局的变量RtpScheduler
```
 *__ortp_scheduler来负责，该变量必须通过ortp_scheduler_init() 来进行初始化操作。
...
// 是否全部发送完毕   
while(curOffset < len span>
    {  
// 如果需要发送的数据长度小于等于阙值，则直接发送 
if( len < BYTES PER_COUNT span>
        {  
            sendBytes = len;  
        }  
else
...
if( curOffset + BYTES_PER_COUNT < len span>
            {  
